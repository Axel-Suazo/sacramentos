...
se encuentra inscrita la partida de confirmación de 
<strong>{{ $persona->nombres }} {{ $persona->apellidos }}</strong>, 
quien recibió el Sacramento de la Confirmación en la Parroquia 
...
<p>
Fue confirmado por el Presbitero
<strong>{{ $sacramento->ministro ?? '—' }}</strong> 
y tuvo como padrinos a 
<strong>{{ $sacramento->padrino1 ?? '—' }}</strong> y 
<strong>{{ $sacramento->padrino2 ?? '—' }}</strong>, 
a quienes se les advirtió sobre su responsabilidad dentro del sacramento.
</p>
